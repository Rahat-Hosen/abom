...
@action("sub_adv_sector/index")
@action.uses("sub_adv_sector/index.html",session,flash)
def index(id=None):
    # if session['status']!='success':
    #     return 'Access Denied'
    sql = """
    SELECT * from sub_adv_sector
    """
...
def validation_check_news_supplement(form):
    ...
    # else:
    #     rows_check=db((db.sub_adv_sector.name==name)).select(db.sub_adv_sector.name,limitby=(0,1))
    #     if rows_check:
    #         form.errors['name'] = ''
...
@action("sub_adv_sector/create", method=['GET', 'POST'])
@action.uses("sub_adv_sector/create.html", session,auth,T)
def create(id=None):  
    db.sub_adv_sector.company.requires=IS_IN_DB(db((db.company.status==1)),db.company.name,error_message='Select a value',zero='Select a value',orderby=db.company.name)
    db.sub_adv_sector.segment.requires=IS_IN_DB(db((db.segment.status==1)),db.segment.name,error_message='Select a value',zero='Select a value',orderby=db.segment.name)    
    db.sub_adv_sector.adv_sector.requires=IS_IN_DB(db((db.segment.status==1)),db.adv_sector.adv_sector,error_message='Select a value',zero='Select a value',orderby=db.adv_sector.adv_sector)    

    form = Form(db.sub_adv_sector,
        fields=['company','segment','adv_sector','sub_adv_sector'],  
        keep_values=True,
    ...
    if 'adv_sector' in form.custom.widgets:
        form.custom.widgets['adv_sector']['_class'] = 'select_custom' 
   
    if form.accepted:
        flash.set('Record added successfully', 'success')
        
        redirect(URL('sub_adv_sector','index'))          
...
@action("sub_adv_sector/edit", method=['GET', 'POST'])
@action.uses("sub_adv_sector/edit.html", session,auth,T)
def edit(id=None): 
    db.sub_adv_sector.company.requires=IS_IN_DB(db((db.company.status==1)),db.company.name,error_message='Select a value',zero='Select a value',orderby=db.company.name)
    db.sub_adv_sector.segment.requires=IS_IN_DB(db((db.segment.status==1)),db.segment.name,error_message='Select a value',zero='Select a value',orderby=db.segment.name)    
    db.sub_adv_sector.adv_sector.requires=IS_IN_DB(db((db.segment.status==1)),db.adv_sector.adv_sector,error_message='Select a value',zero='Select a value',orderby=db.adv_sector.adv_sector)
    record_id = request.query.get('id')
    record= db.sub_adv_sector(record_id) or redirect(URL('sub_adv_sector', 'index'))    
    form = Form(db.sub_adv_sector,
                record=record,
                fields=['company','segment','adv_sector','sub_adv_sector'],    
                keep_values=True,
...
def delete(id=None):
    ...
    record_id = request.query.get('id')
    if record_id:
        db(db.sub_adv_sector.id == record_id).delete()
        
        flash.set('Record Delete successfully', 'error')      
        return dict(redirect(URL('sub_adv_sector', 'index')))

    return locals()


@action("sub_adv_sector/get_data", method=['GET', 'POST'])
def get_data():
    ...
    ##Paginate Start##
    total_rows = len(db.executesql( "SELECT * FROM sub_adv_sector where 1 "+conditions, as_dict=True))
    ...
    ##Querry Start##
    sql = """
    SELECT * FROM `sub_adv_sector` where 1"""+conditions+""" ORDER BY """+sort_column_name+""" """+sort_direction+""" LIMIT """+str(start)+""","""+str(end)+""";
    """
