...
@action.uses("category/index.html",session,flash)
def index(id=None):
...
def validation_check_category(form):
    ...
    if name=='':
        ...
        rows_check=db((db.category.name==name)).select(db.category.name,limitby=(0,1))
        if rows_check:
            form.errors['name'] = ''
...
@action.uses("category/create.html", session,auth,T)
def create(id=None):  
    db.category.company.requires=IS_IN_DB(db((db.company.status==1)),db.company.name,error_message='Select a value',zero='Select a value',orderby=db.company.name)
    db.category.segment.requires=IS_IN_DB(db((db.segment.status==1)),db.segment.name,error_message='Select a value',zero='Select a value',orderby=db.segment.name)    
...
@action.uses("category/edit.html", session,auth,T)
def edit(id=None):            
    record_id = request.query.get('id')
    record= db.category(record_id) or redirect(URL('category', 'index'))    
    db.category.company.requires=IS_IN_DB(db((db.company.status==1)),db.company.name,error_message='Select a value',zero='Select a value',orderby=db.company.name)  
    db.category.segment.requires=IS_IN_DB(db((db.segment.status==1)),db.segment.name,error_message='Select a value',zero='Select a value',orderby=db.segment.name)  
    form = Form(db.category,
                record=record,
...
def delete(id=None):
    ...
    record_id = request.query.get('id')
    if record_id:
        db(db.category.id == record_id).delete()
