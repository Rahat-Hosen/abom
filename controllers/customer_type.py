...
@action("customer_type/index")
@action.uses("customer_type/index.html",session,flash)
def index(id=None):
...
def validation_check_category(form):
    ...
    if name=='':
        ...
        rows_check=db((db.customer_type.name==name)).select(db.customer_type.name,limitby=(0,1))
        if rows_check:
            form.errors['name'] = ''
...
@action("customer_type/create", method=['GET', 'POST'])
@action.uses("customer_type/create.html", session,auth,T)
def create(id=None):  
    db.customer_type.company.requires=IS_IN_DB(db((db.company.status==1)),db.company.name,error_message='Select a value',zero='Select a value',orderby=db.company.name)
    db.customer_type.segment.requires=IS_IN_DB(db((db.segment.status==1)),db.segment.name,error_message='Select a value',zero='Select a value',orderby=db.segment.name)    

    form = Form(db.customer_type,
    ...
    if form.accepted:
        ...
        redirect(URL('customer_type','index'))          
...
@action("customer_type/edit", method=['GET', 'POST'])
@action.uses("customer_type/edit.html", session,auth,T)
def edit(id=None):            
    record_id = request.query.get('id')
    record= db.customer_type(record_id) or redirect(URL('category', 'index'))    
    db.customer_type.company.requires=IS_IN_DB(db((db.company.status==1)),db.company.name,error_message='Select a value',zero='Select a value',orderby=db.company.name)  
    db.customer_type.segment.requires=IS_IN_DB(db((db.segment.status==1)),db.segment.name,error_message='Select a value',zero='Select a value',orderby=db.segment.name)  
...
def delete(id=None):
    ...
    record_id = request.query.get('id')
    if record_id:
        db(db.customer_type.id == record_id).delete()
        
        flash.set('Record Delete successfully', 'error')      
        return dict(redirect(URL('customer_type', 'index')))

    return locals()


@action("customer_type/get_data", method=['GET', 'POST'])
def get_data():
    ...
    ##Querry Start##
    sql = """
    SELECT * FROM `customer_type` where 1"""+conditions+""" ORDER BY """+sort_column_name+""" """+sort_direction+""" LIMIT """+str(start)+""","""+str(end)+""";
    """
