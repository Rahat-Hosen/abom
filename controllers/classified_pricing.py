...
@action("classified_pricing/index")
@action.uses("classified_pricing/index.html",session,flash)
def index(id=None):
    # if session['status']!='success':
    #     return 'Access Denied'
    sql = """
    SELECT * from classified_pricing
    """
...
def validation_check_news_supplement(form):
    ...
    pricing_type=request.forms.get('pricing_type')
    count_words=request.forms.get('count_words')
    first_words=request.forms.get('first_words')
    addition_words=request.forms.get('addition_words')
    errors=[]
    # if name=='':
    #     errors.append('Enter name') 
    # else:
    #     rows_check=db((db.classified_pricing.name==name)).select(db.classified_pricing.name,limitby=(0,1))
    #     if rows_check:
    #         form.errors['name'] = ''
    ...
    if pricing_type=='':
        errors.append('Enter pricing_type')    
    if count_words=='':
        errors.append('Enter count_words')    
    if first_words=='':
        errors.append('Enter first_words')    
    if addition_words=='':
        errors.append('Enter addition_words')    
    if errors:
        msg = ''
        for item in errors:
            msg = msg + item + 'rdrdrd'
...
@action("classified_pricing/create", method=['GET', 'POST'])
@action.uses("classified_pricing/create.html", session,auth,T)
def create(id=None):  
    db.classified_pricing.company.requires=IS_IN_DB(db((db.company.status==1)),db.company.name,error_message='Select a value',zero='Select a value',orderby=db.company.name)
    db.classified_pricing.segment.requires=IS_IN_DB(db((db.segment.status==1)),db.segment.name,error_message='Select a value',zero='Select a value',orderby=db.segment.name)    
    db.classified_pricing.pricing_type.requires=IS_IN_DB(db((db.news_supplement.status==1)),db.news_supplement.name,error_message='Select a value',zero='Select a value',orderby=db.news_supplement.name)    
    form = Form(db.classified_pricing,
        fields=['company','segment','pricing_type','count_words','first_words','addition_words'],  
        keep_values=True,
    ...
    if 'pricing_type' in form.custom.widgets:
        form.custom.widgets['pricing_type']['_class'] = 'select_custom'  
    if form.accepted:
        flash.set('Record added successfully', 'success')
        
        redirect(URL('classified_pricing','index'))          
...
@action("classified_pricing/edit", method=['GET', 'POST'])
@action.uses("classified_pricing/edit.html", session,auth,T)
def edit(id=None): 
    db.classified_pricing.company.requires=IS_IN_DB(db((db.company.status==1)),db.company.name,error_message='Select a value',zero='Select a value',orderby=db.company.name)
    db.classified_pricing.segment.requires=IS_IN_DB(db((db.segment.status==1)),db.segment.name,error_message='Select a value',zero='Select a value',orderby=db.segment.name)    
    db.classified_pricing.pricing_type.requires=IS_IN_DB(db((db.news_supplement.status==1)),db.news_supplement.name,error_message='Select a value',zero='Select a value',orderby=db.news_supplement.name)          
    record_id = request.query.get('id')
    record= db.classified_pricing(record_id) or redirect(URL('classified_pricing', 'index'))    
    form = Form(db.classified_pricing,
                record=record,
                fields=['company','segment','pricing_type'],    
                keep_values=True,
                deletable=True,
...
@action("classified_pricing/delete")
@action.uses("classified_pricing/index.html",session,flash)
def delete(id=None):
    # if session['status']!='success':
    #     return 'Access Denied'
    record_id = request.query.get('id')
    if record_id:
        db(db.classified_pricing.id == record_id).delete()
        
        flash.set('Record Delete successfully', 'error')      
        return dict(redirect(URL('classified_pricing', 'index')))

    return locals()


@action("classified_pricing/get_data", method=['GET', 'POST'])
def get_data():
    ...
    ##Paginate Start##
    total_rows = len(db.executesql( "SELECT * FROM classified_pricing where 1 "+conditions, as_dict=True))
    ...
    ##Querry Start##
    sql = """
    SELECT * FROM `classified_pricing` where 1"""+conditions+""" ORDER BY """+sort_column_name+""" """+sort_direction+""" LIMIT """+str(start)+""","""+str(end)+""";
    """
