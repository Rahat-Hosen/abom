...
@action("brand/index")
@action.uses("brand/index.html",session,flash)
def index(id=None):
...
def validation_check_news_supplement(form):
    ...
    sub_adv_sector=request.forms.get('sub_adv_sector')
    errors=[]
    # if name=='':
    #     errors.append('Enter name') 
    # else:
    #     rows_check=db((db.brand.name==name)).select(db.brand.name,limitby=(0,1))
    #     if rows_check:
    #         form.errors['name'] = ''
    ...
    if sub_adv_sector=='':
        errors.append('Enter sub_adv_sector')    
    if name=='':
        errors.append('Enter name')    
    if advertiser=='':
        errors.append('Enter advertiser')    
    if errors:
        msg = ''
        for item in errors:
            msg = msg + item + 'rdrdrd'
...
@action("brand/create", method=['GET', 'POST'])
@action.uses("brand/create.html", session,auth,T)
def create(id=None):  
    db.brand.company.requires=IS_IN_DB(db((db.company.status==1)),db.company.name,error_message='Select a value',zero='Select a value',orderby=db.company.name)
    db.brand.segment.requires=IS_IN_DB(db((db.segment.status==1)),db.segment.name,error_message='Select a value',zero='Select a value',orderby=db.segment.name)    
    db.brand.sub_adv_sector.requires=IS_IN_DB(db((db.sub_adv_sector.status==1)),db.sub_adv_sector.sub_adv_sector,error_message='Select a value',zero='Select a value',orderby=db.sub_adv_sector.sub_adv_sector)    
    db.brand.advertiser.requires=IS_IN_DB(db((db.advertiser.status==1)),db.advertiser.advertiser,error_message='Select a value',zero='Select a value',orderby=db.advertiser.advertiser)    

    form = Form(db.brand,
        fields=['company','segment','sub_adv_sector','advertiser','name'],  
        keep_values=True,
    ...
    if 'advertiser' in form.custom.widgets:
        form.custom.widgets['advertiser']['_class'] = 'select_custom' 
    if 'sub_adv_sector' in form.custom.widgets:
        form.custom.widgets['sub_adv_sector']['_class'] = 'select_custom' 
...
@action("brand/edit", method=['GET', 'POST'])
@action.uses("brand/edit.html", session,auth,T)
def edit(id=None): 
    db.brand.company.requires=IS_IN_DB(db((db.company.status==1)),db.company.name,error_message='Select a value',zero='Select a value',orderby=db.company.name)
    db.brand.segment.requires=IS_IN_DB(db((db.segment.status==1)),db.segment.name,error_message='Select a value',zero='Select a value',orderby=db.segment.name)    
    db.brand.sub_adv_sector.requires=IS_IN_DB(db((db.sub_adv_sector.status==1)),db.sub_adv_sector.sub_adv_sector,error_message='Select a value',zero='Select a value',orderby=db.sub_adv_sector.sub_adv_sector)    
    ...
    record= db.brand(record_id) or redirect(URL('brand', 'index'))    
    form = Form(db.brand,
                record=record,
                fields=['company','segment','sub_adv_sector','advertiser','name'],    
                keep_values=True,
...
def delete(id=None):
    ...
    record_id = request.query.get('id')
    if record_id:
        db(db.brand.id == record_id).delete()
        
        flash.set('Record Delete successfully', 'error')      
        return dict(redirect(URL('brand', 'index')))

    return locals()


@action("brand/get_data", method=['GET', 'POST'])
def get_data():
    ...
    ##Paginate Start##
    total_rows = len(db.executesql( "SELECT * FROM brand where 1 "+conditions, as_dict=True))
    ...
    ##Querry Start##
    sql = """
    SELECT * FROM `brand` where 1"""+conditions+""" ORDER BY """+sort_column_name+""" """+sort_direction+""" LIMIT """+str(start)+""","""+str(end)+""";
    """
