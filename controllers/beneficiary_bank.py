...
@action("beneficiary_bank/index")
@action.uses("beneficiary_bank/index.html",session,flash)
def index(id=None):
    # if session['status']!='success':
    #     return 'Access Denied'
    sql = """
    SELECT * from beneficiary_bank
    """
...
def validation_check_category(form):
    ...
    bank_id=request.forms.get('bank_id')
    bank_name=request.forms.get('bank_name')
    errors=[]
    # if receipt_mode=='':
    #     errors.append('Enter receipt_mode') 
    # else:
    #     rows_check=db((db.beneficiary_bank.beneficiary_bank==beneficiary_bank)).select(db.beneficiary_bank.beneficiary_bank,limitby=(0,1))
    #     if rows_check:
    #         form.errors['beneficiary_bank'] = ''
    #         errors.append('beneficiary_bank already exist') 
    ...
    if receipt_mode=='':
        errors.append('Enter receipt_mode')
    if bank_id=='':
        errors.append('Enter bank_id')
    if bank_name=='':
        errors.append('Enter bank_name')
    if errors:
        msg = ''
        for item in errors:
            msg = msg + item + 'rdrdrd'
...
@action("beneficiary_bank/create", method=['GET', 'POST'])
@action.uses("beneficiary_bank/create.html", session,auth,T)
def create(id=None):  
    db.beneficiary_bank.company.requires=IS_IN_DB(db((db.company.status==1)),db.company.name,error_message='Select a value',zero='Select a value',orderby=db.company.name)
    db.beneficiary_bank.segment.requires=IS_IN_DB(db((db.segment.status==1)),db.segment.name,error_message='Select a value',zero='Select a value',orderby=db.segment.name)    
    db.beneficiary_bank.receipt_mode.requires=IS_IN_DB(db((db.receipt_mode.status==1)),db.receipt_mode.receipt_mode,error_message='Select a value',zero='Select a value',orderby=db.receipt_mode.receipt_mode)    
    form = Form(db.beneficiary_bank,
        fields=['receipt_mode','company','segment','bank_id','bank_name','status'],   
        keep_values=True,
    ...
    if 'receipt_mode' in form.custom.widgets:
        form.custom.widgets['receipt_mode']['_class'] = 'select_custom'   
    if form.accepted:
        flash.set('Record added successfully', 'success')
        
        redirect(URL('beneficiary_bank','index'))          
...
@action("beneficiary_bank/edit", method=['GET', 'POST'])
@action.uses("beneficiary_bank/edit.html", session,auth,T)
def edit(id=None):            
    record_id = request.query.get('id')
    record= db.beneficiary_bank(record_id) or redirect(URL('beneficiary_bank', 'index'))    
    db.beneficiary_bank.company.requires=IS_IN_DB(db((db.company.status==1)),db.company.name,error_message='Select a value',zero='Select a value',orderby=db.company.name)  
    db.beneficiary_bank.segment.requires=IS_IN_DB(db((db.segment.status==1)),db.segment.name,error_message='Select a value',zero='Select a value',orderby=db.segment.name)  
    db.beneficiary_bank.receipt_mode.requires=IS_IN_DB(db((db.receipt_mode.status==1)),db.receipt_mode.receipt_mode,error_message='Select a value',zero='Select a value',orderby=db.receipt_mode.receipt_mode)    
    form = Form(db.beneficiary_bank,
...
@action("beneficiary_bank/delete")
@action.uses("beneficiary_bank/index.html",session,flash)
def delete(id=None):
    # if session['status']!='success':
    #     return 'Access Denied'
    record_id = request.query.get('id')
    if record_id:
        db(db.beneficiary_bank.id == record_id).delete()
        
        flash.set('Record Delete successfully', 'error')      
        return dict(redirect(URL('beneficiary_bank', 'index')))

    return locals()


@action("beneficiary_bank/get_data", method=['GET', 'POST'])
def get_data():
    ...
    ##Paginate Start##
    total_rows = len(db.executesql( "SELECT * FROM beneficiary_bank where 1 "+conditions, as_dict=True))
    ...
    ##Querry Start##
    sql = """
    SELECT * FROM `beneficiary_bank` where 1"""+conditions+""" ORDER BY """+sort_column_name+""" """+sort_direction+""" LIMIT """+str(start)+""","""+str(end)+""";
    """
