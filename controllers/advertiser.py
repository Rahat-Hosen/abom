...
@action("advertiser/index")
@action.uses("advertiser/index.html",session,flash)
def index(id=None):
...
def validation_check_news_supplement(form):
    ...
    # else:
    #     rows_check=db((db.advertiser.name==name)).select(db.advertiser.name,limitby=(0,1))
    #     if rows_check:
    #         form.errors['name'] = ''
...
@action("advertiser/create", method=['GET', 'POST'])
@action.uses("advertiser/create.html", session,auth,T)
def create(id=None):  
    db.advertiser.company.requires=IS_IN_DB(db((db.company.status==1)),db.company.name,error_message='Select a value',zero='Select a value',orderby=db.company.name)
    db.advertiser.segment.requires=IS_IN_DB(db((db.segment.status==1)),db.segment.name,error_message='Select a value',zero='Select a value',orderby=db.segment.name)    
    db.advertiser.sub_adv_sector.requires=IS_IN_DB(db((db.sub_adv_sector.status==1)),db.sub_adv_sector.sub_adv_sector,error_message='Select a value',zero='Select a value',orderby=db.sub_adv_sector.sub_adv_sector)    

    form = Form(db.advertiser,
        fields=['company','segment','sub_adv_sector','advertiser'],  
        keep_values=True,
...
@action("advertiser/edit", method=['GET', 'POST'])
@action.uses("advertiser/edit.html", session,auth,T)
def edit(id=None): 
    db.advertiser.company.requires=IS_IN_DB(db((db.company.status==1)),db.company.name,error_message='Select a value',zero='Select a value',orderby=db.company.name)
    db.advertiser.segment.requires=IS_IN_DB(db((db.segment.status==1)),db.segment.name,error_message='Select a value',zero='Select a value',orderby=db.segment.name)    
    db.advertiser.sub_adv_sector.requires=IS_IN_DB(db((db.sub_adv_sector.status==1)),db.sub_adv_sector.sub_adv_sector,error_message='Select a value',zero='Select a value',orderby=db.sub_adv_sector.sub_adv_sector)    
    record_id = request.query.get('id')
    record= db.advertiser(record_id) or redirect(URL('advertiser', 'index'))    
    form = Form(db.advertiser,
                record=record,
                fields=['company','segment','sub_adv_sector','advertiser'],    
                keep_values=True,
...
def delete(id=None):
    ...
    record_id = request.query.get('id')
    if record_id:
        db(db.advertiser.id == record_id).delete()
        
        flash.set('Record Delete successfully', 'error')      
        return dict(redirect(URL('advertiser', 'index')))

    return locals()


@action("advertiser/get_data", method=['GET', 'POST'])
def get_data():
    ...
    ##Paginate Start##
    total_rows = len(db.executesql( "SELECT * FROM advertiser where 1 "+conditions, as_dict=True))
    ...
    ##Querry Start##
    sql = """
    SELECT * FROM `advertiser` where 1"""+conditions+""" ORDER BY """+sort_column_name+""" """+sort_direction+""" LIMIT """+str(start)+""","""+str(end)+""";
    """
